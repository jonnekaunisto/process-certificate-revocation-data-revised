...
Filter.create = function create(elements, falsePositiveRate, level) {
  ...
  // See: https://github.com/bitcoin/bitcoin/blob/master/src/bloom.cpp
  //console.log(falsePositiveRate);
  //var size = -1.0 / Filter.LN2SQUARED * elements * Math.log(falsePositiveRate);
  //console.log("old size " + size);
  //var nHashFuncs = Math.ceil(size / elements * Filter.LN2);
  //console.log("old hash " + nHashFuncs);
  // var nHashFuncs = Math.ceil(Math.log(1.0 / falsePositiveRate) / Math.log(2));
  var nHashFuncs = 2;
  //console.log("new hash " + nHashFuncs);
  var size = Math.ceil(1 - (nHashFuncs * (elements + 0.5) / Math.log(1 - (Math.pow(falsePositiveRate, (1 / nHashFuncs))))));
...
Filter.fromJSON = function(data) {
  ...
  data.vData = new Buffer(data.vData, 'base64');
